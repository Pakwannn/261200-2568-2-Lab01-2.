...
public class Snowman implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
...
//        t.width(3);
    ...
    public class Snowman {
        ...
        // ฟังก์ชันวาดวงกลม
        static void circle(Snowman t, int radius) {
            for (int i = 0; i < 360; i++) {
        ...
        public static void main(String[] a) {
            ...
            t.up();
            t.left(90);
            t.forward(25);   // ลงมาที่คอระหว่างหัวกับลำตัวบน
            t.right(90);
            t.backward(30);
            t.down();

            // คาดรอบคอ (สี่เหลี่ยมผืนผ้า)
            for (int i = 0; i < 2; i++) {
                t.forward(60);
                t.left(90);
                t.forward(15);
...
//        Harry.penColor("green"); // change pen color
//        Harry.width(5);
//        Harry.speed(0.05);
//
//        for(int i = 0; i < 6; i++) {
...
//    }
    ...
    private static ArrayList<Snowman> turtles;
    ...
    private static HashMap<Snowman,ArrayList<ArrayList>> mouseBindings;
    ...
    private static Snowman selectedTurtle;
    ...
    private static void init()
    {
        ...
        mouseBindings=new HashMap<Snowman,ArrayList<ArrayList>>();
        centerX=0;
        centerY=0;
        scale=1;
...
//        System.out.println(Arrays.toString(fontNames));
    ...
    private static void retrieveState(long time)
    {
        synchronized(turtleLock)
        {
            Snowman t=getStateTurtle(turtleStates.get(time));
            t.__time=t._time;
    ...
    private static Snowman getStateTurtle(ArrayList state){return (Snowman)state.get(1);}
    ...
    {
        Color c= Snowman.getColor(penColor);
        if(c!=null)this.penColor=c;
        long timeStamp=storeCurrentState();
        return timeStamp;
    }
    ...
    {
        Color c= Snowman.getColor(outlineColor);
    ...
    {
        Color c= Snowman.getColor(fillColor);
    ...
    {
        Color c= Snowman.getColor(color);
    ...
    private static boolean addMouseBinding(String methodName, Snowman t, boolean append, boolean click, boolean repeat)
    {
        String className="";
        try
        {
    ...
    public static void zoomFit()
    {
        synchronized(turtleLock)
        {
            ...
            if(turtleStates.lastKey()>time && getStateSpeed(turtleStates.lastEntry().getValue())>0)
            {
                ...
                Snowman t=getStateTurtle(turtleStates.lastEntry().getValue());
    ...
    private static void draw()
    {
        synchronized(turtleLock)
        {
            ...
            for (Map.Entry<Long, ArrayList> entry : turtleStates.tailMap(lastUpdate).headMap(renderTime).entrySet())
            {
                ...
                Snowman t=getStateTurtle(entry.getValue());
    ...
    private void processKeys()
    {
        ...
        for(String keyText:keysDownCopy)
        {
            if (keyBindings.containsKey(keyText))
            {
                ...
                {
                    Snowman t=(Snowman)binding.get(0);
